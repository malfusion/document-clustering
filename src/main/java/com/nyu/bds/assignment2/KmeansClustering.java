...
public class KmeansClustering {
	...
	public void initKMeanPlusCentroids() {
		...
		for (int i = 1; i < numClusters; i++) {
			...
			// At this point, we will know the farthest point which can become the new
			// centroid.
	...
	public double[][] updateCentroids(HashMap<String, Integer> files_centroids) {
		...
		for (int i = 0; i < numClusters; i++) {
			...
			newCentroids[i] = similarityMeasure
					.getAverageVector(vectors.toArray(new double[vectors.size()][features.length]));
		}
		return newCentroids;
	}
	...
	public HashMap<String, Integer> assign() {
		...
		for (String filePath : wordTfidfByFile.keySet()) {
			...
			for (int i = 0; i < numClusters; i++) {
				// double distance = getEuclideanDistance(centroids[i],
				// wordTfidfByFile.get(filePath));
